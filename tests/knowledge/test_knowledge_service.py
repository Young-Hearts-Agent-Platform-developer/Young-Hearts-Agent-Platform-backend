...
def test_list_items(service, db_session, user_id):
    for i in range(5):
        service.create_item(user_id=user_id, data=KnowledgeItemCreate(title=f"标题{i}", content=f"内容{i}"))
    total, items = service.list_items(status=None, skip=0, limit=10)
    assert total == 5
    assert len(items) == 5
